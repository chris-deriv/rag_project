...
def test_add_documents_with_cleanup(mock_chroma_client):
    ...
    with patch('chromadb.PersistentClient', return_value=mock_client):
        ...
        def mock_get(**kwargs):
            ...
            source_name = where.get('source_name', {}).get('$eq')
            if source_name == 'test1.pdf':
                return {
                    'ids': ['old1'],
                    'metadatas': [{'source_name': 'test1.pdf'}],
                    'documents': ['Old doc 1']
                }
            elif source_name == 'test2.docx':
                return {
                    'ids': [],
                    'metadatas': [],
                    'documents': []
...
def test_add_documents_with_inconsistent_chunks(mock_chroma_client):
    ...
    with patch('chromadb.PersistentClient', return_value=mock_client):
        ...
        with pytest.raises(ValueError) as exc_info:
            db.add_documents(documents)
        assert "Inconsistent total_chunks values" in str(exc_info.value)
...
def test_query_with_title_filter(mock_chroma_client):
    ...
    with patch('chromadb.PersistentClient', return_value=mock_client):
        ...
        assert call_kwargs['where']['title'] == {'$eq': 'python'}
