...
@pytest.fixture
def mock_vector_db():
    with patch('src.api.vector_db') as mock_db:
        yield mock_db

@pytest.fixture
def mock_rag_app():
    with patch('src.api.rag_app') as mock_app:
        yield mock_app

@pytest.fixture
def mock_add_document():
    with patch('src.api.add_document') as mock_add:
        yield mock_add

@pytest.fixture
def mock_get_documents():
    with patch('src.api.get_documents') as mock_get:
        yield mock_get

@pytest.fixture
def mock_document_store():
    with patch('src.api.document_store') as mock_store:
